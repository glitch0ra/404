...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ─────── НОВАЯ ФУНКЦИЯ ОТРИСОВКИ СИМВОЛОВ 0 И 1 ───────
    float pixelChar(vec2 U, vec2 seed, float highlight) {
        // Определяем, какой символ рисовать (0 или 1) на основе seed
        float symbolSelect = hash(seed);
        bool isOne = symbolSelect > 0.5;
        
        // Пиксельный стиль: имитация низкого разрешения
        vec2 pixelGrid = floor(U * vec2(5.0, 8.0)) / vec2(5.0, 8.0);
        U = pixelGrid + fract(U * vec2(5.0, 8.0)) * 0.2; // Сглаживание пикселей
        
        float base = 0.0;
        float glow = 0.0;
        
        if (isOne) {
            // Символ '1' - вертикальная полоса с основанием
            float verticalBar = smoothstep(0.25, 0.2, abs(U.x - 0.5));
            float basePlate = smoothstep(0.1, 0.05, U.y) * smoothstep(0.1, 0.05, 1.0 - U.y);
            
            // Основная часть символа
            base = verticalBar;
            // Добавляем основание только в нижней части
            base = max(base, basePlate * step(0.7, U.y));
            
            // Эффект подсветки для вертикальной части
            glow = highlight * smoothstep(0.35, 0.3, abs(U.x - 0.5));
        } else {
            // Символ '0' - прямоугольник с закругленными углами
            vec2 center = vec2(0.5);
            vec2 rel = abs(U - center);
            float cornerRadius = 0.2;
            
            // Расстояние до краев прямоугольника
            vec2 d = abs(rel - vec2(0.4)) - vec2(0.3);
            float rectDist = length(max(d, 0.0)) + min(max(d.x, d.y), 0.0);
            
            // Внутренний прямоугольник для обводки
            vec2 d_inner = abs(rel - vec2(0.4)) - vec2(0.2);
            float rectDist_inner = length(max(d_inner, 0.0)) + min(max(d_inner.x, d_inner.y), 0.0);
            
            // Основная обводка символа
            base = smoothstep(0.05, 0.0, -rectDist) - smoothstep(0.05, 0.0, -rectDist_inner);
            base = clamp(base, 0.0, 1.0);
            
            // Эффект подсветки
            glow = highlight * smoothstep(0.15, 0.1, -rectDist);
        }
        
        // Объединяем основной символ и подсветку
        return clamp(base + glow, 0.0, 1.0);
    }
    
    float random_char(vec2 outer, vec2 inner, float highlight) {
        vec2 seed = vec2(dot(outer, vec2(269.5, 183.3)), dot(outer, vec2(113.5, 271.9)));
        return pixelChar(inner, seed, highlight);
    }

    // ─────── ОСТАВШАЯСЯ ЧАСТЬ ШЕЙДЕРА БЕЗ ИЗМЕНЕНИЙ ───────
    vec3 rain(vec3 ro3, vec3 rd3, float time) {
        ...
        float localTime = mod(time, 500.0); // Адаптивное количество итераций
                    ...
                    if (abs(u) < target_rad) {
                        ...
                        if (v >= 0.0 && v < 1.0) {
                            ...
                            if (char_hash.x >= 0.1 || c == 0.) {
                                ...
                                if (a > 0.) {
                                    float attenuation = 1. + pow(0.06 * tmin / t3_to_t2, 2.0);
    ...
    let mouseX = 0, mouseY = 0; // Отслеживание мыши
