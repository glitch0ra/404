  ...
  vec3 rain(vec3 ro3, vec3 rd3, float time) {
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        for (int j = 0; j < 2; j++) {
            ...
            float dist = tmin / t3_to_t2; // расстояние вдоль луча

            // 🚫 пропускаем всё ближе чем 4.0 (чем больше, тем “дальше”)
                    ...
                    if (v >= 0.0 && v < 1.0) {
                        ...
                        if (char_hash.x >= 0.1 || c == 0.) {
                            float time_hash = hash(vec2(time * 0.1 + cell_hash2.x * 7.3, cell_hash2.y * 13.1));
                                  float time_factor = floor(c == 0. ? mod(time*5.0 + time_hash*100.0, 1000.0) :
                                  mod(time*(1.0*cell_hash2.z + cell_hash2.w*cell_hash2.w*4.*pow(char_hash.y, 4.)) + time_hash*500.0, 1000.0));
                            float a = random_char(vec2(char_hash.x, time_factor), vec2(u, q), max(1., 3. - c / 2.) * 0.2);
                            a *= clamp((chars_count - 0.5 - c) / 2., 0., 1.);

                            // 🌫️ плавное затухание по глубине
