...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ГЛОБАЛЬНЫЕ КОНСТАНТЫ ДЛЯ СЛОЁВ
    const float PLANE_DIST = 1.0;
    const int FURTHEST = 16;
    const int FADEFROM = 10;
    const float FADEDIST = PLANE_DIST * float(FADEFROM);
    const float MAXDIST = PLANE_DIST * float(FURTHEST);
    ...
    // ИЗМЕНЁННАЯ ФУНКЦИЯ ДЛЯ СЛОЁВ (добавлен layerId)
    vec4 plane(vec3 ro, vec3 rd, vec3 pp, vec3 npp, vec3 off, float n, float layerId) {
        float h = hash(n);
        vec2 p = (pp - off*2.0*vec3(1.0,1.0,0.0)).xy;
        const vec2 stp = vec2(0.5, 0.33);
        
        // Вычисляем расстояние один раз для оптимизации
        ...
        // Базовое сглаживание краёв
        float aa = distance(pp, npp)*sqrt(1.0/3.0);
        
        // --- РАЗМЫТИЕ ДЛЯ ДАЛЬНИХ СЛОЁВ (12-16) ---
        if (layerId >= 12.0) {
            // Коэффициент размытия зависит от расстояния (максимальный в момент появления)
            float blurFactor = (MAXDIST - dist) / MAXDIST;
            blurFactor = clamp(blurFactor, 0.0, 1.0);
            
            // Дополнительный вес по номеру слоя (16й слой размывается сильнее 12го)
            float layerBlur = (layerId - 11.0) / 5.0; // 12->0.2, 16->1.0
            
            // Применяем адаптивное размытие
            aa *= (1.0 + blurFactor * layerBlur * 6.0);
        }
        // ----------------------------------------
        
        float t = smoothstep(aa, -aa, d);
        float df = exp(-0.1 * (dist - 2.0)); // Используем предвычисленное расстояние
    ...
    vec3 color(vec3 ww, vec3 uu, vec3 vv, vec3 ro, vec2 p, out float outA) {
        ...
        float nz = floor(ro.z / PLANE_DIST); // Используем глобальную константу
        vec4 accum = vec4(0.0);
        
        // Цикл по слоям с использованием глобальных констант
        for (int i = 1; i <= FURTHEST; ++i) {
            ...
            if (pp.y < 0.0 && pd > 0.0 && accum.w < 0.95) {
                ...
                // --- ИСПРАВЛЕНА ЛОГИКА ПЛАВНОГО ПОЯВЛЕНИЯ ---
                float fadeIn = 1.0 - smoothstep(FADEDIST, MAXDIST, pd);
                
                // Передаём номер слоя (i) в plane()
