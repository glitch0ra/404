...
(() => {
  ...
  const fragSrc = `
    ...
    float NetLayer(vec2 st, float n, float t){
        ...
        { vec2 p = p0; m += line(p4,p,st); float d = length(st - p); float s = 0.002/(d*d + 0.0001); s *= S(1.0,0.1,d); float pulse = sin((fract(p.x)+fract(p.y)+t)*5.0)*0.4+0.6; pulse = pow(pulse,20.0); s *= pulse; sparkle += s; }
        { vec2 p = p1; m += line(p4,p,st); float d = length(st - p); float s = 0.002/(d*d + 0.0001); s *= S(1.0,0.1,d); float pulse = sin((fract(p.x)+fract(p.y)+t)*5.0)*0.4+0.6; pulse = pow(pulse,20.0); s *= pulse; sparkle += s; }
        { vec2 p = p2; m += line(p4,p,st); float d = length(st - p); float s = 0.002/(d*d + 0.0001); s *= S(1.0,0.1,d); float pulse = sin((fract(p.x)+fract(p.y)+t)*5.0)*0.4+0.6; pulse = pow(pulse,20.0); s *= pulse; sparkle += s; }
        { vec2 p = p3; m += line(p4,p,st); float d = length(st - p); float s = 0.002/(d*d + 0.0001); s *= S(1.0,0.1,d); float pulse = sin((fract(p.x)+fract(p.y)+t)*5.0)*0.4+0.6; pulse = pow(pulse,20.0); s *= pulse; sparkle += s; }
        { vec2 p = p4; m += line(p4,p,st); float d = length(st - p); float s = 0.002/(d*d + 0.0001); s *= S(1.0,0.1,d); float pulse = sin((fract(p.x)+fract(p.y)+t)*5.0)*0.4+0.6; pulse = pow(pulse,20.0); s *= pulse; sparkle += s; }
        { vec2 p = p5; m += line(p4,p,st); float d = length(st - p); float s = 0.002/(d*d + 0.0001); s *= S(1.0,0.1,d); float pulse = sin((fract(p.x)+fract(p.y)+t)*5.0)*0.4+0.6; pulse = pow(pulse,20.0); s *= pulse; sparkle += s; }
        { vec2 p = p6; m += line(p4,p,st); float d = length(st - p); float s = 0.002/(d*d + 0.0001); s *= S(1.0,0.1,d); float pulse = sin((fract(p.x)+fract(p.y)+t)*5.0)*0.4+0.6; pulse = pow(pulse,20.0); s *= pulse; sparkle += s; }
        { vec2 p = p7; m += line(p4,p,st); float d = length(st - p); float s = 0.002/(d*d + 0.0001); s *= S(1.0,0.1,d); float pulse = sin((fract(p.x)+fract(p.y)+t)*5.0)*0.4+0.6; pulse = pow(pulse,20.0); s *= pulse; sparkle += s; }
        { vec2 p = p8; m += line(p4,p,st); float d = length(st - p); float s = 0.002/(d*d + 0.0001); s *= S(1.0,0.1,d); float pulse = sin((fract(p.x)+fract(p.y)+t)*5.0)*0.4+0.6; pulse = pow(pulse,20.0); s *= pulse; sparkle += s; }
