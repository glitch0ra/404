...
(() => {
  ...
  // === Вершинный шейдер (стандартный для полноэкранного квада) ===
  const vertSrc = `
  ...
  // === Фрагментный шейдер (ваш новый шейдер с рунами) ===
  const fragSrc = `
    ...
    // ===== ВСТАВЛЕННЫЙ ШЕЙДЕР НАЧИНАЕТСЯ ЗДЕСЬ =====
                        ...
                        if (v >= 0.0 && v < 1.0) {
                            ...
                            if (char_hash.x >= 0.1 || c == 0.) {
                                float time_factor = floor(c == 0. ? time*5.0 :
                                    time*(1.0*cell_hash2.z + cell_hash2.w*cell_hash2.w*4.*pow(char_hash.y, 4.)));
    ...
    void main() {
        ...
        // Преобразуем vUv из [0,1] в Shadertoy-стиль: fragCoord = vUv * iResolution.xy
        vec2 fragCoord = vUv * iResolution.xy;

        // Основной код из mainImage
        ...
        vec2 dir;
        if (tq < 0.25) {
            prev = vec2(0.,0.); dir = vec2(0.,1.);
        } else if (tq < 0.5) {
            prev = vec2(0.,1.); dir = vec2(1.,0.);
        } else if (tq < 0.75) {
            prev = vec2(1.,1.); dir = vec2(0.,-1.);
        } else {
            prev = vec2(1.,0.); dir = vec2(-1.,0.);
        }
  ...
  // Настройка буфера вершин (полный экран)
  ...
  const updateMouse = (e) => {
    ...
    mouseY = (rect.height - (e.clientY - rect.top)) * dpr; // инвертируем Y для Shadertoy-совместимости
    isMouseActive = true;
  };

  window.addEventListener('mousemove', updateMouse);
  window.addEventListener('mousedown', updateMouse);
  window.addEventListener('mouseup', () => { isMouseActive = false; });
  ...
  function render(now) {
    ...
    // iMouse: (x, y, isDown ? 1 : 0, 0) — Shadertoy передаёт z=1 при нажатии
    gl.uniform4f(uniMouse, mouseX, mouseY, isMouseActive ? 1.0 : 0.0, 0.0);
