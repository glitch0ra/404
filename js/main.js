...
const int ITERATIONS = 30;   // меньше — быстрее
...
// === ЦИФРОВОЙ ДОЖДЬ (упрощённый — фиксированная камера) ===
vec3 rain(vec3 ro3, vec3 rd3, float time) {
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        // Пропускаем "пробелы" между блоками (опционально — можно убрать)
        // Но для простоты оставим плотный дождь: закомментируем gap-логику
        // (иначе будут пустые полосы)

        float t3s = t2s / t3_to_t2;
        float pos_z = ro3.z + rd3.z * t3s;
        float xycell_hash = hash(vec2(cell));
        float z_shift = xycell_hash * 11.0 - time * (0.5 + xycell_hash + xycell_hash * xycell_hash + pow(xycell_hash, 16.0) * 3.0);
                    ...
                    if (v >= 0.0 && v < 1.0) {
                        ...
                        if (char_hash.x >= 0.1 || c == 0.0) {
                            ...
                            if (a > 0.0) {
                                ...
                                vec3 col = vec3(1.0) / attenuation; // БЕЛЫЙ цвет
                                float a1 = result.a;
                                result.a = a1 + (1.0 - a1) * a;
                                result.xyz = (result.xyz * a1 + col * (1.0 - a1) * a) / (result.a + 1e-6);
...
// === MAIN ===
void main() {
    // Преобразуем vUv (0..1) в координаты экрана
    vec2 fragCoord = vUv * iResolution.xy;

    // Фиксированная камера: смотрим вдаль по Z (как в стандартном Matrix rain)
    vec2 uv = (2.0 * fragCoord - iResolution.xy) / iResolution.y;

    float time = iTime * SPEED;

    // Позиция камеры: чуть выше земли
    vec3 ro = vec3(0.0, 0.0, 0.0);
    // Луч: направление вперёд-вниз (чтобы видеть падающие символы)
    vec3 rd = normalize(vec3(uv.x, uv.y, -1.0)); // смотрим "вниз" по Z

    // Смещаем луч немного вперёд, чтобы не начинать с камеры
    ro += rd * 0.1;

    vec3 col = rain(ro, rd, time);

    // Прозрачность (если нужно)
    float alpha = min(1.0, length(col) * 1.5);
